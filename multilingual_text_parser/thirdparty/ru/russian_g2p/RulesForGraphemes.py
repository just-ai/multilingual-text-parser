...
class RulesForGraphemes:
    def __init__(self, users_mode: str = "Modern"):
        if users_mode == "Classic":
            from multilingual_text_parser.thirdparty.ru.russian_g2p.modes.Classic import (
                ClassicMode as UsersMode,
            )
        else:
            from multilingual_text_parser.thirdparty.ru.russian_g2p.modes.Modern import (
                ModernMode as UsersMode,
            )
    ...
    def apply_rule_for_vocals(self, letters_list: list, cur_pos: int) -> list:
        ...
        new_phonemes_list.append(
            self.mode.TableG2P[letters_list[cur_pos]].forms["case" + str(case)]
        )
        return new_phonemes_list

    def apply_rule_for_consonants(
        self, letters_list: list, next_phoneme: str, cur_pos: int
    ) -> list:
        new_phonemes_list = []
        n = len(letters_list)
        # твердость / мягкость
