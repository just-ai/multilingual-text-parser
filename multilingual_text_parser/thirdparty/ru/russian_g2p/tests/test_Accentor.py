...
class TestRussianAccentor1(unittest.TestCase):
    ...
    def test_do_accents_positive10(self):
        ...
        target_variants = [["о+зеро", "та+к", "серебри+сто", "в", "све+те", "со+лнца"]]
        real_variants = self.__accentor.do_accents(source_phrase)
        self.assertEqual(target_variants, real_variants)
    ...
    def test_calculate_morpho_similarity_positive01(self):
        self.assertAlmostEqual(
            self.__accentor.calculate_morpho_similarity("1", "a,b"), 0.0, places=7
        )
        self.assertAlmostEqual(
            self.__accentor.calculate_morpho_similarity("a,b c,d,e", "a,b c,d,e"),
            1.0,
            places=7,
        )
        self.assertAlmostEqual(
            self.__accentor.calculate_morpho_similarity("a,b c,d,e", "f,g h,i,j"),
            0.0,
            places=7,
        )
        self.assertAlmostEqual(
            self.__accentor.calculate_morpho_similarity("a,b c,d,e", "f,b h,d,j"),
            0.25,
            places=7,
        )
        self.assertAlmostEqual(
...
class TestRussianAccentor2(unittest.TestCase):
    ...
    def test_do_accents_positive01(self):
        source_phrase = [["оружие"], ["для"], ["кубы"]]
        target_variants = [["ору+жие", "для", "ку+бы"], ["ору+жие", "для", "кубы+"]]
        real_variants = self.__accentor.do_accents(source_phrase)
        self.assertEqual(target_variants, real_variants)
