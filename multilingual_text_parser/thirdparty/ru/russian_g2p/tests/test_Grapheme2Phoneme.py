...
from thirdparty.russian_g2p.Grapheme2Phoneme import Grapheme2Phoneme
...
class TestRussianG2P(unittest.TestCase):
    ...
    def test_phrase_to_phonemes_positive_tlp0_001(self):
        ...
        self.assertEqual(
            ["A", "F", "T", "A", "B0", "I0", "Z", "N", "Y", "S"],
            self.__g2p.word_to_phonemes("автоби+знес"),
        )
        self.assertEqual(
            ["D", "Y", "K", "A", "L0", "T", "E0"],
            self.__g2p.word_to_phonemes("декольте+"),
        )
        self.assertEqual(
            [
                "M",
    ...
    def test_phrase_to_phonemes_positive_tlp1_001(self):
        ...
        self.assertEqual(
            ["TSH0", "A0", "S", "N", "Y", "J0"], self.__g2p.word_to_phonemes("ча+стный")
        )
        self.assertEqual(
            ["SH0", "I", "S", "L0", "I0", "V", "Y", "J0"],
            self.__g2p.word_to_phonemes("счастли+вый"),
        )
        self.assertEqual(
            ["R0", "I", "N", "G0", "E0", "N"], self.__g2p.word_to_phonemes("рентге+н")
        )
    ...
    def test_phrase_to_phonemes_positive_tlp2_001(self):
        ...
        self.assertEqual(
            ["P", "O0", "Z", "N", "A"], self.__g2p.word_to_phonemes("по+здно")
        )
        self.assertEqual(["U", "S", "TS", "Y0"], self.__g2p.word_to_phonemes("уздцы+"))
        # self.assertEqual(['G', 'A', 'Ll', 'A0', 'N', 'TS', 'Y'], self._g2p.word_to_phonemes('голла+ндцы'))
        self.assertEqual(
            ["G", "A", "L", "A0", "N", "TS", "Y"],
            self.__g2p.word_to_phonemes("голла+ндцы"),
        )
        self.assertEqual(
            ["S0", "E0", "R", "TS", "Y"], self.__g2p.word_to_phonemes("се+рдце")
        )
        self.assertEqual(
            ["L", "A", "N", "SH", "A0", "F", "T"],
            self.__g2p.word_to_phonemes("ландша+фт"),
        )
        self.assertEqual(
            ["J0", "I", "K", "T", "A0", "SH"], self.__g2p.word_to_phonemes("ягдта+ш")
        )
    ...
    def test_phrase_to_phonemes_positive_tlp3_001(self):
        ...
        self.assertEqual(
            ["S", "O0", "N", "TS", "Y"], self.__g2p.word_to_phonemes("со+лнце")
        )
    ...
    def test_phrase_to_phonemes_positive_tlp4_001(self):
        ...
        self.assertEqual(
            ["P0", "I", "R0", "I", "V", "A", "L", "N", "A", "V", "A0", "TS", "A"],
            self.__g2p.word_to_phonemes("переволнова+ться"),
        )
        self.assertEqual(
            ["R", "U", "TSH0", "A0", "J0", "I", "TS", "A"],
            self.__g2p.word_to_phonemes("руча+ется"),
        )
    ...
    def test_phrase_to_phonemes_positive_tlp5_001(self):
        ...
        self.assertEqual(
            ["K", "R", "A0", "S", "N", "A", "V", "A"],
            self.__g2p.word_to_phonemes("кра+сного"),
        )
        self.assertEqual(
            ["S0", "I0", "N0", "I", "V", "A"], self.__g2p.word_to_phonemes("си+него")
        )
    ...
    def test_phrase_to_phonemes_positive_tlp6_001(self):
        ...
        self.assertEqual(
            ["A", "N0", "T0", "I0", "KH", "R0", "I", "S", "T"],
            self.__g2p.word_to_phonemes("анти+христ"),
        )
        self.assertEqual(
            ["B", "A", "N0", "D0", "U0", "G", "A"],
            self.__g2p.word_to_phonemes("бандю+га"),
        )
        ...
        self.assertEqual(
            ["P", "U", "S0", "T0", "A0", "K"], self.__g2p.word_to_phonemes("пустя+к")
        )

    def test_phrase_to_phonemes_positive_tlp6_002(self):
        """Проверка правила ПБФ27."""
        self.assertEqual(
            ["V0", "I", "Z0", "D0", "E0"], self.__g2p.word_to_phonemes("везде+")
        )
        self.assertEqual(["S0", "T0", "E0", "P0"], self.__g2p.word_to_phonemes("сте+пь"))
        self.assertEqual(
            ["SH0", "I", "T", "A0", "L", "S0", "A"],
            self.__g2p.word_to_phonemes("счита+лся"),
        )
        self.assertEqual(
            ["J0", "E0", "S", "L0", "I"], self.__g2p.word_to_phonemes("е+сли")
        )
    ...
    def test_phrase_to_phonemes_positive_tlp7_001(self):
        ...
        self.assertEqual(
            ["U0", "S0", "T0", "J0", "I"], self.__g2p.phrase_to_phonemes("у+стье")
        )
        self.assertEqual(
            ["M", "A0", "R0", "J0", "A"], self.__g2p.phrase_to_phonemes("Ма+рья")
        )
        self.assertEqual(
            ["K", "A", "E0", "L0", "J0", "A"], self.__g2p.phrase_to_phonemes("Коэ+льо")
        )
        self.assertEqual(
            ["B", "Y0", "TSH0", "J0", "I"], self.__g2p.phrase_to_phonemes("бы+чьи")
        )
        self.assertEqual(
            ["B0", "E0", "L", "A", "J0", "A"], self.__g2p.phrase_to_phonemes("бе+лая")
        )
    ...
    def test_phrase_to_phonemes_positive_tlp7_002(self):
        ...
        self.assertEqual(
            ["R", "A", "Z", "J0", "O0", "M"], self.__g2p.phrase_to_phonemes("разъё+м")
        )
        self.assertEqual(
            ["P", "A", "D", "J0", "U0", "P", "K", "A", "J0"],
            self.__g2p.phrase_to_phonemes("под ю+бкой"),
        )
        self.assertEqual(
            ["B", "U", "L0", "J0", "O0", "N"], self.__g2p.phrase_to_phonemes("бульо+н")
        )
        self.assertEqual(
            ["J0", "I", "M", "SH0", "I0", "K"], self.__g2p.phrase_to_phonemes("ямщи+к")
        )
        self.assertEqual(
            ["V", "Y0", "B", "A", "R"], self.__g2p.phrase_to_phonemes("вы+бор")
        )
        self.assertEqual(
            ["A", "O0", "R", "T", "A"], self.__g2p.phrase_to_phonemes("ао+рта")
        )
    ...
    def test_phrase_to_phonemes_positive_tlp7_003(self):
        ...
        self.assertEqual(
            ["K0", "I", "SH0", "A0", "N"], self.__g2p.phrase_to_phonemes("Кещя+н")
        )
        self.assertEqual(
            ["J0", "A0", "R", "TSH0", "I"], self.__g2p.phrase_to_phonemes("я+рче")
        )
        self.assertEqual(
            ["G", "A", "S", "P", "A0", "TSH0", "A"],
            self.__g2p.phrase_to_phonemes("гаспа+чо"),
        )
        self.assertEqual(
            ["U", "D", "A0", "TSH0", "A"], self.__g2p.phrase_to_phonemes("уда+ча")
        )

    def test_phrase_to_phonemes_positive_tlp7_004(self):
        """Checking line 4 for vowels."""
        self.assertEqual(
            ["M", "A", "J0", "A0", "M0", "I"], self.__g2p.phrase_to_phonemes("майа+ми")
        )
        self.assertEqual(
            ["TSH0", "O0", "R", "N", "Y", "J0"],
            self.__g2p.phrase_to_phonemes("чё+рный"),
        )
        self.assertEqual(
            ["TSH0", "I", "S", "T", "O0", "T", "Y"],
            self.__g2p.phrase_to_phonemes("часто+ты"),
        )
    ...
    def test_phrase_to_phonemes_positive_tlp7_005(self):
        ...
        self.assertEqual(
            ["N", "R", "A0", "V0", "I", "TS", "A"],
            self.__g2p.phrase_to_phonemes("нра+вится"),
        )
        self.assertEqual(
            ["S", "T", "R0", "I", "ZH", "Y0"], self.__g2p.phrase_to_phonemes("стрижи+")
        )
    ...
    def test_phrase_to_phonemes_positive_tlp7_006(self):
        ...
        self.assertEqual(
            ["TS", "E0", "N", "T", "R"], self.__g2p.phrase_to_phonemes("це+нтр")
        )
        self.assertEqual(
            ["L", "U", "TS", "Y", "K0", "I0"], self.__g2p.phrase_to_phonemes("Луцяки+")
        )
        self.assertEqual(
            ["SH", "A", "SH", "L", "Y0", "K"], self.__g2p.phrase_to_phonemes("шашлы+к")
        )
        self.assertEqual(
            ["SH", "A", "K0", "I0", "R", "A", "V", "A", "T0"],
            self.__g2p.phrase_to_phonemes("шоки+ровать"),
        )
        self.assertEqual(
            ["SH", "Y", "P", "T", "A0", "T0"], self.__g2p.phrase_to_phonemes("шепта+ть")
        )
        self.assertEqual(
            ["ZH", "Y", "V", "O0", "J0"], self.__g2p.phrase_to_phonemes("живо+й")
        )
    ...
    def test_phrase_to_phonemes_positive_tlp7_008(self):
        ...
        self.assertEqual(
            ["L0", "I", "G", "U0", "SH", "K", "A"],
            self.__g2p.phrase_to_phonemes("лягу+шка"),
        )
        self.assertEqual(
            ["M", "A", "R", "O0", "S"], self.__g2p.phrase_to_phonemes("моро+з")
        )
    ...
    def test_phrase_to_phonemes_positive_tlp8_001(self):
        ...
        self.assertEqual(
            ["M0", "I0", "T0", "K", "A"], self.__g2p.phrase_to_phonemes("Ми+тька")
        )

    def test_phrase_to_phonemes_positive_tlp8_002(self):
        """Checking line 2 for consonants in the middle."""
        self.assertEqual(
            ["D0", "J0", "A0", "K", "A", "N"], self.__g2p.phrase_to_phonemes("дья+кон")
        )
        self.assertEqual(
            ["S", "A", "N0", "T0", "J0", "A0", "G", "A"],
            self.__g2p.phrase_to_phonemes("Сантья+го"),
        )

    def test_phrase_to_phonemes_positive_tlp8_003(self):
        """Checking line 3 for consonants in the middle."""
        self.assertEqual(
            ["S", "V", "A0", "D0", "B", "A"], self.__g2p.phrase_to_phonemes("сва+дьба")
        )
        self.assertEqual(
            ["ZH", "Y", "N0", "I0", "D0", "B", "A"],
            self.__g2p.phrase_to_phonemes("жени+тьба"),
        )
    ...
    def test_phrase_to_phonemes_positive_tlp8_004(self):
        ...
        self.assertEqual(
            ["T0", "E0", "R0", "I", "M"], self.__g2p.phrase_to_phonemes("те+рем")
        )

    def test_phrase_to_phonemes_positive_tlp8_005(self):
        """Checking line 5 for consonants in the middle."""
        self.assertEqual(
            ["N", "A", "K", "T0", "O0", "M"], self.__g2p.phrase_to_phonemes("ногтё+м")
        )
        self.assertEqual(
            ["L", "A", "K", "T0", "O0", "M"], self.__g2p.phrase_to_phonemes("локтё+м")
        )
    ...
    def test_phrase_to_phonemes_positive_tlp8_007(self):
        ...
        self.assertEqual(
            ["V", "A", "G", "Z", "A0", "L"], self.__g2p.phrase_to_phonemes("вокза+л")
        )
    ...
    def test_phrase_to_phonemes_positive_tlp8_008(self):
        ...
        self.assertEqual(
            ["K", "O0", "M0", "I", "K"], self.__g2p.phrase_to_phonemes("ко+мик")
        )
    ...
    def test_phrase_to_phonemes_positive_tlp8_010(self):
        ...
        self.assertEqual(
            ["S", "K", "O0", "R", "P0", "S", "T", "A0", "R", "A", "J0", "A"],
            self.__g2p.phrase_to_phonemes("ско+рбь ста+рая"),
        )
        self.assertEqual(
            ["G", "R", "U0", "S0", "T0", "P", "R0", "I", "SH", "L", "A0"],
    ...
    def test_phrase_to_phonemes_positive_tlp8_011(self):
        ...
        self.assertEqual(
            ["S", "K", "O0", "R", "P0", "V0", "E0", "TSH0", "N", "A", "J0", "A"],
            self.__g2p.phrase_to_phonemes("ско+рбь ве+чная"),
        )
        self.assertEqual(
            ["G", "R", "U0", "S0", "T0", "V0", "E0", "TSH0", "N", "A", "J0", "A"],
    ...
    def test_phrase_to_phonemes_positive_tlp8_012(self):
        ...
        self.assertEqual(
            ["S", "K", "O0", "R", "B0", "G", "O0", "R0", "K", "A", "J0", "A"],
            self.__g2p.phrase_to_phonemes("ско+рбь го+рькая"),
        )
        self.assertEqual(
            ["G", "R", "U0", "Z0", "D0", "G", "O0", "R0", "K", "A", "J0", "A"],
    ...
    def test_phrase_to_phonemes_positive_tlp8_013(self):
        ...
        self.assertEqual(
            ["S", "K", "O0", "R", "P0", "U", "ZH", "E0"],
            self.__g2p.phrase_to_phonemes("ско+рбь уже+"),
        )
        self.assertEqual(
            ["G", "R", "U0", "S0", "T0", "U", "ZH", "E0"],
            self.__g2p.phrase_to_phonemes("гру+сть уже+"),
        )

    def test_phrase_to_phonemes_positive_tlp8_014(self):
        """Checking line 6 for consonants in the end."""
        self.assertEqual(
            ["G", "R0", "I0", "P", "S", "V0", "E0", "ZH", "Y", "J0"],
            self.__g2p.phrase_to_phonemes("гри+б све+жий"),
        )
        self.assertEqual(
            ["T", "R", "U0", "P", "S", "V0", "E0", "ZH", "Y", "J0"],
    ...
    def test_phrase_to_phonemes_positive_tlp8_015(self):
        ...
        self.assertEqual(
            ["G", "R0", "I0", "P", "V0", "A0", "L", "Y", "J0"],
            self.__g2p.phrase_to_phonemes("гри+б вя+лый"),
        )
        self.assertEqual(
            ["T", "R", "U0", "P", "V0", "A0", "L", "Y", "J0"],
    ...
    def test_phrase_to_phonemes_positive_tlp8_016(self):
        ...
        self.assertEqual(
            ["G", "R0", "I0", "B", "G", "A", "R0", "A0", "TSH0", "I", "J0"],
            self.__g2p.phrase_to_phonemes("гри+б горя+чий"),
        )
        self.assertEqual(
            ["T", "R", "U0", "B", "G", "A", "R0", "A0", "TSH0", "I", "J0"],
    ...
    def test_phrase_to_phonemes_positive_tlp8_017(self):
        ...
        self.assertEqual(
            ["G", "R0", "I0", "P", "U", "ZH", "E0"],
            self.__g2p.phrase_to_phonemes("гри+б уже+"),
        )
        self.assertEqual(
            ["T", "R", "U0", "P", "U", "ZH", "E0"],
            self.__g2p.phrase_to_phonemes("тру+п уже+"),
        )
    ...
    def test_phrase_to_phonemes_positive_tlp9_001(self):
        ...
        #                  self._g2p.word_to_phonemes('интеллиге+нт'))
        self.assertEqual(
            ["I", "N0", "T0", "I", "L0", "I", "G0", "E0", "N", "T"],
            self.__g2p.word_to_phonemes("интеллиге+нт"),
        )
        # self.assertEqual(['K', 'A', 'M0', 'I0', 'S0l', 'I', 'J0', 'A'], self._g2p.word_to_phonemes('коми+ссия'))
        self.assertEqual(
            ["K", "A", "M0", "I0", "S0", "I", "J0", "A"],
            self.__g2p.word_to_phonemes("коми+ссия"),
        )
        # self.assertEqual(['M', 'A', 'S0l', 'I0', 'F'], self._g2p.word_to_phonemes('масси+в'))
        self.assertEqual(
            ["M", "A", "S0", "I0", "F"], self.__g2p.word_to_phonemes("масси+в")
        )
        # self.assertEqual(['A', 'T0l', 'A0', 'SH', 'K', 'A'], self._g2p.word_to_phonemes('оття+жка'))
        self.assertEqual(
            ["A", "T0", "A0", "SH", "K", "A"], self.__g2p.word_to_phonemes("оття+жка")
        )
        # self.assertEqual(['A', 'S0l', 'I', 'S0', 'T0', 'E0', 'N', 'T'], self._g2p.word_to_phonemes('ассисте+нт'))
        self.assertEqual(
            ["A", "S0", "I", "S0", "T0", "E0", "N", "T"],
            self.__g2p.word_to_phonemes("ассисте+нт"),
        )
    ...
    def test_phrase_to_phonemes_positive_tlp10_001(self):
        """Проверка TLP 10."""
        self.assertEqual(
            ["B", "L0", "U0", "TS", "Y"], self.__g2p.word_to_phonemes("блю+дце")
        )
        self.assertEqual(["A", "TS", "A0"], self.__g2p.word_to_phonemes("отца+"))
        self.assertEqual(
            ["R", "A", "SH", "Y0", "P"], self.__g2p.word_to_phonemes("расши+б")
        )
        self.assertEqual(
            ["V", "J0", "I", "ZH", "A0", "T0"], self.__g2p.word_to_phonemes("въезжа+ть")
        )

    def test_word_to_phonemes_positive029(self):
        """Проверка корректности обработки прописных букв."""
        self.assertEqual(
            self.__g2p.word_to_phonemes("ё+жик"), self.__g2p.word_to_phonemes("Ё+жик")
        )
    ...
    def test_word_to_phonemes_positive030(self):
        ...
        self.assertEqual(
            self.__g2p.word_to_phonemes("ё+жик"), self.__g2p.word_to_phonemes("ёжик")
        )
        self.assertEqual(
            self.__g2p.word_to_phonemes("ё+жик"), self.__g2p.word_to_phonemes("Ёжик")
        )
    ...
    def test_word_to_phonemes_positive031(self):
        ...
        self.assertEqual(
            ["SH", "A", "F0", "O0", "R"], self.__g2p.word_to_phonemes("шофё+р")
        )

    def test_word_to_phonemes_positive032(self):
        """Баг https://github.com/nsu-ai/russian_g2p/issues/8."""
        self.assertEqual(
            ["J0", "I0", "TSH0", "N0", "I", "TS", "U"],
            self.__g2p.word_to_phonemes("яи+чницу"),
        )
    ...
    def test_word_to_phonemes_positive033(self):
        ...
        self.assertEqual(
            ["A", "B0", "I", "S0", "I0", "N0", "I"],
            self.__g2p.word_to_phonemes("абисси+нии"),
        )
    ...
    def test_word_to_phonemes_negative002(self):
        ...
        target_error_message = (
            "`интерде+вочка!`: this word contains inadmissible characters!"
        )
        with self.assertRaisesRegex(AssertionError, re.escape(target_error_message)):
            self.__g2p.word_to_phonemes(source_word)

    def test_word_to_phonemes_negative003(self):
        """Генерация исключения, если слово содержит только неалфавитные символы, хоть и
        допустимые."""
        source_word = "+-"
    ...
    def test_word_to_phonemes_negative004(self):
        """Генерация предупреждения, если слово указано без ударения и содержит более
        одной гласной."""
        source_word = "через"
    ...
    def test_phrase_to_phonemes_positive001(self):
        """Проверка корректной работы правил межсловного преобразования на стыке
        ...
        self.assertEqual(
            ["N0", "I", "D0", "E0", "L", "A", "T0"],
            self.__g2p.phrase_to_phonemes("не де+лать"),
        )
    ...
    def test_phrase_to_phonemes_positive002(self):
        ...
        """
        self.assertEqual(
            ["P", "A", "SH", "O0", "L", "T", "A", "K0", "I"],
            self.__g2p.phrase_to_phonemes("пошё+л-таки"),
        )
    ...
    def test_phrase_to_phonemes_positive003(self):
        ...
        """
        # self.assertEqual(['I', 'Zl', 'A', 'T', 'U0', 'TSH0'], self._g2p.phrase_to_phonemes('из-за ту+ч'))
        self.assertEqual(
            ["I", "Z", "A", "T", "U0", "TSH0"],
            self.__g2p.phrase_to_phonemes("из-за ту+ч"),
        )
    ...
    def test_phrase_to_phonemes_positive004(self):
        ...
        self.assertEqual(
            ["P", "A", "D", "A", "R", "O0", "G0", "I"],
            self.__g2p.phrase_to_phonemes("по доро+ге"),
        )
        self.assertEqual(
            ["N", "A", "S", "T", "A", "L0", "E0"],
            self.__g2p.phrase_to_phonemes("на столе+"),
        )
    ...
    def test_phrase_to_phonemes_positive005(self):
        ...
        Для первого слова: первое слово заканчивается согласной, проверка ассимиляции
        по глухости-звонкости.

        """
        self.assertEqual(
            ["F", "S", "A", "D", "U0"], self.__g2p.phrase_to_phonemes("в саду+")
        )
        self.assertEqual(
            ["V", "L0", "I", "S", "U0"], self.__g2p.phrase_to_phonemes("в лесу+")
        )
        self.assertEqual(
            ["Z", "D", "A", "R", "O0", "G0", "I"],
            self.__g2p.phrase_to_phonemes("с доро+ги"),
        )
        self.assertEqual(
            ["S", "V", "A0", "S0", "I", "J0"], self.__g2p.phrase_to_phonemes("с Ва+сей")
        )
    ...
    def test_phrase_to_phonemes_positive006(self):
        ...
        Для первого слова: первое слово заканчивается твёрдой согласной, которая не
        должна переходить в мягкую фонему перед гласными Я, Ё, Ю, Е, И.

        """
        self.assertEqual(
            ["P", "A", "D", "J0", "O0", "L", "K", "A", "J0"],
            self.__g2p.phrase_to_phonemes("под ё+лкой"),
        )
    ...
    def test_phrase_to_phonemes_positive007(self):
        ...
        Для первого слова: первое слово заканчивается согласной, и она ассимилируется
        с последующей мягкой согласной, одинаковой по месту образования.

        """
        self.assertEqual(
            ["B0", "I", "Z0", "D0", "E0", "D", "A"],
            self.__g2p.phrase_to_phonemes("без де+да"),
        )
    ...
    def test_phrase_to_phonemes_positive008(self):
        ...
        Для второго слова: первое слово заканчивается мягким знаком, а второе слово
        начинается с гласной О или с гласной И, и эти гласные ни в коем случае не
        должны переходить в сочетания фонем [J O] или [J I] ([J I0], если под
        ударением) соответственно.

        """
        self.assertEqual(
            ["I", "L0", "O0", "S0", "I", "N0"],
            self.__g2p.phrase_to_phonemes("иль о+сень"),
        )  # выдает O
        self.assertEqual(
            ["I", "L0", "I0", "V", "A", "L", "G", "A"],
            self.__g2p.phrase_to_phonemes("иль и+волга"),
        )
    ...
    def test_phrase_to_phonemes_positive009(self):
        ...
        Для второго слова: второе слово начинается с гласной И, которая должна
        переходить в фонему [Y] (или [Y0], если под ударением) после всех согласных,
        кроме Й.

        """
        self.assertEqual(
            ["P", "A", "D", "Y0", "V", "A", "J0"],
            self.__g2p.phrase_to_phonemes("под и+вой"),
        )
        self.assertEqual(
            ["S", "Y", "V", "A0", "N", "A", "M"],
            self.__g2p.phrase_to_phonemes("с Ива+ном"),
        )
        # для второго слова: если первое слово заканчивается мягким знаком, то согласная смягчается, но не оглушается
        self.assertEqual(
            ["V0", "I", "T0", "I", "V", "A0", "N"],
            self.__g2p.phrase_to_phonemes("ведь Ива+н"),
        )

    def test_phrase_to_phonemes_positive010(self):
        """Проверка корректной работы правил межсловного преобразования на стыке двух
        знаменательных слов.
    ...
    def test_phrase_to_phonemes_positive015(self):
        ...
        self.assertEqual(
            ["J0", "I", "D", "R0", "I0", "T0", "K", "A", "L", "A", "T0", "I0", "T0"],
    ...
    def test_phrase_to_phonemes_positive016(self):
        ...
        self.assertEqual(
            ["O0", "KH", "O0", "KH", "O0", "KH"],
            self.__g2p.phrase_to_phonemes("о+х-о+х-о+х"),
        )
        self.assertEqual(
            ["B", "A0", "J0", "U", "SH", "K0", "I", "B", "A", "J0", "U0"],
            self.__g2p.phrase_to_phonemes("ба+юшки-баю+"),
        )
        self.assertEqual(
            ["T", "R", "A0", "KH", "T", "A0", "KH", "T", "A0", "KH"],
            self.__g2p.phrase_to_phonemes("тра+х-та+х-та+х"),
        )
        self.assertEqual(
            ["T", "R0", "E0", "N0", "B", "R0", "E0", "N0"],
            self.__g2p.phrase_to_phonemes("тре+нь-бре+нь"),
        )

    def test_phrase_to_phonemes_positive017(self):
        """Проверка корректности транскрибирования целых фраз, состоящих более чем из двух
        слов.
    ...
    def test_phrase_to_phonemes_positive018(self):
        ...
        Простое предложение с двумя словами-фонетическими исключениями ("мужчины" и
        "декольте").
        ...
        #     self._g2p.phrase_to_phonemes('поцелу+й мужчи+ны в ще+дро откры+тое же+нское декольте+ во+все не счита+лся '
        #                                   'неприли+чным')
        # )
        self.assertEqual(
            [
            ...
            self.__g2p.phrase_to_phonemes(
                "поцелу+й мужчи+ны в ще+дро откры+тое же+нское декольте+ во+все не счита+лся "
                "неприли+чным"
            ),
        )

    def test_phrase_to_phonemes_positive019(self):
        """Проверка стыка знаменательного и служебного слов в ситуации, когда служебное
        слово идёт вторым."""
        self.assertEqual(
            ["J0", "E0", "S", "L0", "I", "B", "Y", "N0", "I", "T", "Y0"],
